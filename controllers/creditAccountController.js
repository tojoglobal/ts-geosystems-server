...
export const deleteCreditAccountApplication = async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await db.query(
      "DELETE FROM credit_account_applications WHERE id = ?",
      [id]
    );
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ message: "Credit account application not found." });
    }
    res
      .status(200)
      .json({ message: "Credit account application deleted successfully." });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error deleting credit account application." });
  }
};